...
async function loadSystemZip(file){
  if(!file) return;
  showLoading(); setLoadingMsg('Reading zip…');
  ...
  try{
    ...
    refreshTexPickerLists();
    updateAssetEmptyState();
    setTimeout(() => { hideLoading(); hideLoadingBars(); setLoadingTitle('LOADING SYSTEM'); goNew(); setTimeout(() => { drawViewport(); }, 500); }, 350);

...
function calculateHabitabilityZone() {
  ...
  // Get difficulty multiplier — read from the app's actual difficulty toggle
  // (viewDiffKey/_DEF_SMA_SCALE, set by cycleDifficulty() in viewport.js), not a
  // '#difficulty-select' dropdown or 'DIFFICULTY_MULTIPLIERS' map — neither of
  // those exist anywhere in the app, so this used to throw a ReferenceError the
  // instant a star type was picked, before results ever rendered.
  const difficultyKey = (typeof viewDiffKey !== 'undefined') ? viewDiffKey : 'Normal';
  const _diffScale = (typeof _DEF_SMA_SCALE !== 'undefined') ? _DEF_SMA_SCALE : {Normal:1, Hard:2, Realistic:20};
  const diffMult = _diffScale[difficultyKey] ?? 1;
  ...
  // Planet files key orbit data under ORBIT_DATA, not ORBIT — this previously
  // always read undefined and fell back to 0, so every body silently showed as
  // "not orbiting" regardless of its actual semi-major axis.
  const sma = (body?.ORBIT_DATA?.semiMajorAxis || 0) / 1e9; // to Gm
  ...
  const hz_center_gm = hz_center_au * 149.597870700;

  document.getElementById('hab-hz-inner').textContent = `${hz_inner_gm.toFixed(2)} Gm`;
  document.getElementById('hab-hz-center').textContent = `${hz_center_gm.toFixed(2)} Gm`;
  ...
  // Draw visualization
  drawHabitabilityVisualization(hz_inner_gm, hz_center_gm, hz_outer_gm, sma);
}
